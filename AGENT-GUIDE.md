...
| configure 在 RC / `CMakeRCCompiler.cmake` 出錯 | 沒傳 `CMAKE_RC_COMPILER`，或傳入的路徑含空格。若 `$RC` 仍含空格，代表該磁碟停用了 8.3 短檔名（ROCm wheel 不含 `llvm-rc`，不能拿來替代）。**首選**（不改系統設定）：把 `rc.exe` 連同同目錄的 `rcdll.dll` 複製到無空格的資料夾，例如 `New-Item -ItemType Directory C:\tools\winrc -Force; Copy-Item (Join-Path (Split-Path $rcLong) 'rc.exe'),(Join-Path (Split-Path $rcLong) 'rcdll.dll') C:\tools\winrc\`，再傳 `-DCMAKE_RC_COMPILER=C:/tools/winrc/rc.exe`，並刪掉 `build` 目錄重跑 configure。**次選**：開啟 8.3 短檔名。這需要系統管理員權限，也會改動系統設定，**必須先徵得使用者同意** |
